...
float GetShadow
(
    const float3 ShadowCoord, const float4 ShadowTexelSize,
    uniform Texture2D<float> texShadow, SamplerComparisonState shadowSampler
)
{
#ifdef SINGLE_SAMPLE
...
{
    // GetShadow( const float3 ShadowCoord, const float4 ShadowTexelSize,
    // uniform Texture2D texShadow, SamplerComparisonState shadowSampler )
    float shadow = GetShadow(shadowCoord, ShadowTexelSize, texShadow, shadowSampler);
...
float3 ApplyConeShadowedLight(
    ...
    uint lightIndex,
    Texture2DArray<float> lightShadowArrayTex,
    SamplerComparisonState shadowSampler
    )
{
